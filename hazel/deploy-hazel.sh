...
SHARE="${ASCEND_SHARE:-/share/$UNITY}"
...
echo "==> running install.sh on the login node (file install into ~/bin, ~/.claude, $SHARE/agents)"
ssh "$REMOTE" "ASCEND_SCRATCH='$SHARE/agents' ASCEND_SITE_NAME=ASCEND-HAZEL bash ~/$STAGE/install.sh"
