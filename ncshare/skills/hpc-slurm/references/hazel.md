...
- 2026-09-09: full harness smoke PASSED on login03 (job 776118, partition
  compute, account <pi>_cpu): probe -> workspace -> revision -> validate ->
...
- Accounts on this allocation follow the PI’s Unity ID: `<pi>_cpu`, `<pi>_gpu` (run `sacctmgr show assoc user=$USER format=account` to list yours).
